...
export default function Customers() {
  ...
  const fetchVehicles = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/vehicles`);
      const data = await res.json();
      setVehicles(data || []);
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateDetails = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/leads/${selectedCustomer.id}`, {
        method: "PUT",
      ...
      if (res.ok) {
        if (editIsSold && selectedVehicleId && markSoldInInventory) {
          ...
          if (matched) {
            await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/finance/sales`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({

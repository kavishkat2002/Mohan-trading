...
export default function Dashboard() {
  ...
  const fetchDashboardData = async (silent = false) => {
    ...
    try {
      ...
      try {
        const profileRes = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/users/${localId}/profile`);
        if (profileRes.ok) {
      ...
      const noticeRes = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/notices`);
      ...
      if (user?.role === 'owner' || user?.role === 'admin') {
        ...
        const [leadsRes, vehiclesRes, attendanceRes, tasksRes, usersRes, salesRes, attSummaryRes] = await Promise.all([
          ...
          fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/all`),
          fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/tasks?userId=${localId}&role=${user.role}`),
          fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/users`),
          ...
          fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/dashboard-summary`)
        ]);
        ...
        const [leadsRes, tasksRes, commRes, attRes] = await Promise.all([
          ...
          fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/status/${localId}`)
        ]);
      ...
      try {
        const statsRes = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/messages/stats`);
        if (statsRes.ok) {

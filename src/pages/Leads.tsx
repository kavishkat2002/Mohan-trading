...
export default function Leads() {
  ...
  const handleEditLead = async (e: React.FormEvent) => {
    ...
    if (!editLead) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/leads/${editLead.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editLead.name,
  ...
  const handleAssignLead = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/leads/${assignLead.id}/assign`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
  ...
  const handleCommissionClose = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/leads/${commissionLead.id}/commission`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const updateLeadStatus = async (lead: any, newStatus: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/leads/${lead.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: lead.name,

...
export default function ChatPage() {
  ...
  const fetchMessages = async (lead: any) => {
    ...
    try {
      ...
      if (lead.phone) {
        ...
        if (supaLead) {
          ...
          if (supaMessages && supaMessages.length > 0) {
            await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/messages/sync`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/messages/lead/${lead.id}`);
      if (res.ok) {
        const data = await res.json();
        setMessages(data);
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/messages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteConversation = async () => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/leads/${selectedLead.id}`, { method: "DELETE" });
      if (supaLeadId) {

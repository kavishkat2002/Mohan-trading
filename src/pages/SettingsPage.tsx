...
function TeamManager({ isOwner }: { isOwner: boolean }) {
  ...
  const fetchUsers = () => {
    fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/admin/users`)
      .then(res => res.json())
      .then(data => {
  ...
  const addMember = async (e: React.FormEvent) => {
    ...
    setIsAdding(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/admin/create-user`, {
        method: "POST",
  ...
  const changeRole = async (supabaseId: string, role: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/admin/update-role/${supabaseId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const deleteMember = async (supabaseId: string) => {
    setIsDeleting(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/admin/delete-user/${supabaseId}`, { method: "DELETE" });
      if (res.ok) {
...
export default function SettingsPage() {
  ...
  const saveVehicleNotes = async (id: number, notes: string) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/vehicles/${id}/ai-notes`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/users/${user?.id || 1}/avatar`, {
        method: "POST",
        body: formData
      });
      const data = await res.json();
        ...
        <TabsContent value="business" className="mt-6">
          <SectionCard title="Business Profile" desc="Update your business identity and contact details">
            ...
            <div className="flex flex-col items-center justify-center pb-3 pt-1">
              ...
              >
                {logoUrl ? (
                  <img src={`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}${logoUrl.replace(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`, '')}`} alt="Logo" className="h-full w-full object-contain p-1.5" />
                ) : (
                  ...
                  <form 
                    onSubmit={async (e) => {
                      ...
                      try {
                        ...
                        const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}/api/settings/test-chat`, {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({

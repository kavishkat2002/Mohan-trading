...
export default function Vehicles() {
  ...
  const generateCatalog = () => {
    ...
    const rows = inStockVehicles.map(v => {
      const imgSrc = v.image_url ? `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}${v.image_url}` : '';
      return `
        <div class="card">
          <div class="card-img">
                ...
                <form onSubmit={handleAddVehicle} className="space-y-4 py-4">
                    ...
                    {additionalImages.length > 0 ? (
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {editingVehicle.additional_images.map((url: string, idx: number) => (
                          <img key={idx} src={`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}${url}`} alt="existing" className="w-16 h-16 object-cover rounded-md border border-border opacity-70" />
                        ))}
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {imageFile ? (
                      ...
                      <div className="mt-2">
                        <img src={`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}${editingVehicle.image_url}`} alt="existing" className="w-16 h-16 object-cover rounded-md border border-border opacity-70" />
                      </div>
                    ) : null}
                  </div>
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map(v => (
                      <TableRow key={v.id} className="hover:bg-primary/[0.02] transition-colors border-border">
                        <TableCell>
                          {v.image_url ? (
                            <img src={`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}${v.image_url}`} alt="car" className="w-14 h-10 object-cover rounded-md border border-border" />
                          ) : (
                ...
                filtered.map(v => (
                  <div key={v.id} className="p-4 flex gap-4 hover:bg-primary/[0.02] transition-colors">
                    ...
                    <div className="w-20 h-16 shrink-0 relative">
                      {v.image_url ? (
                        <img src={`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5001'}`}${v.image_url}`} alt="car" className="w-full h-full object-cover rounded-lg border border-border" />
                      ) : (

...
export default function TestDrives() {
  ...
  const fetchData = async (silent = false) => {
    try {
      ...
      const [leadsRes, vehiclesRes, bookingsRes] = await Promise.all([
        ...
        fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/test-drives`)
      ]);
  ...
  const handleCreateBooking = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/test-drives`, {
        method: "POST",
  ...
  const handleUpdateStatus = async (bookingId: number, status: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/test-drives/${bookingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },

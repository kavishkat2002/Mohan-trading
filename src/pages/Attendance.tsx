...
export default function Attendance() {
  ...
  const fetchStatus = async (dateStr: string) => {
    if (!user) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/status/${user.id}?date=${dateStr}&email=${encodeURIComponent(user.email)}`);
  ...
  const fetchAdminData = async () => {
    try {
      const [attRes, leavesRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}/api/attendance/all`),
        fetch(`${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}/api/attendance/leaves`)
      ]);
  ...
  const fetchMyData = async () => {
    ...
    try {
      const [attRes, leavesRes] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/my-attendance/${user.id}?email=${encodeURIComponent(user.email)}`),
        fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/my-leaves/${user.id}?email=${encodeURIComponent(user.email)}`)
      ]);
      const attData = await attRes.json();
      const leavesData = await leavesRes.json();
  ...
  const handleCheckIn = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/check-in`, {
            method: "POST",
  ...
  const handleCheckOut = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/check-out`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_id: user?.id, email: user?.email, lat: pos.coords.latitude, lng: pos.coords.longitude })
          });
  ...
  const submitLeaveRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/leaves`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const updateLeaveStatus = async (id: number, status: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'https://runny-unsentiently-teresita.ngrok-free.dev'}`}/api/attendance/leaves/${id}/status`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },

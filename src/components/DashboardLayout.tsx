...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const checkSubscription = () => {
      fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/subscription`)
        .then(res => res.json())
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/sync`, {
      method: "POST",
      ...
      .then(localUser => {
        ...
        if (!isElevated) {
          fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/${localId}/commissions`)
            .then(res => res.json())
  ...
  useEffect(() => {
    ...
    const fetchNotifications = () => {
      fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/${localUserId}/notifications`)
        .then(res => res.json())
        .then(data => {
  ...
  const handleMarkAsRead = async (notifId: number) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/notifications/${notifId}/read`, {
        method: "PUT"
      });
      if (res.ok) {
  ...
  const handleMarkAllAsRead = async () => {
    if (!localUserId) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/${localUserId}/notifications/read-all`, {
        method: "PUT"
      });
      if (res.ok) {
        setNotifications([]);
...
function ProfileSettingsDialog({ open, onOpenChange, user, localUserId, setProfileName, setProfileAvatar }: { open: boolean; onOpenChange: (open: boolean) => void; user: any; localUserId: number | null; setProfileName: (n: string) => void; setProfileAvatar: (a: string) => void; }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/${localUserId}/avatar`, {
        method: "POST",
        body: formData
      });
  ...
  useEffect(() => {
    if (open && localUserId) {
      fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/${localUserId}/profile`)
        .then(res => res.json())
        .then(data => {
          if (data.name) {
            setName(data.name);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:5001`}`}/api/users/${localUserId}/profile`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name,
